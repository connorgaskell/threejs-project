...
THREE.Terrain.ScatterMeshes = function(geometry, options) {
    ...
    for (var i = 0, w = Math.floor(options.w * 1); i < w; i++) {
        for (var j = 0, h = options.h; j < h; j++) {
            var key = 0, f = geometry.faces[(w * 2) * j + (i * 2)], place = false;
            // j * w + i
            /*if (spreadIsNumber) {
                console.log(key);
            ...
            if (place) {
                ...
                /*
                 * Get mesh based on tile coordinates
                 */
                var mesh = options.mesh.clone();
                mesh.geometry.computeBoundingBox();

                //mesh.position.copy(v[f.a]).add(v[f.b]).add(v[f.c]).divideScalar(3);
                mesh.position.copy(v[f.b]).add(v[f.c]).divideScalar(2);

                console.log("x: " + ((mesh.position.x / 200) + 1) + " | y: " + ((mesh.position.y / 200) + 1));
